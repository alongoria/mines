...
void list::operator=(const list& source) {
	...
	node* source_list = this->_head;
	node* curren_t = source._head;
	for (node* curren_t = source._head->_next; curren_t != NULL; curren_t = curren_t->_next) {
		node* new_node = new node(curren_t->_data);
		source_list->_next = new_node;
		source_list = source_list->_next;
	}
